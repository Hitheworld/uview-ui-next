...
import { ComponentInternalInstance, withDefaults, getCurrentInstance, ref, DefineComponent } from 'vue'
  ...
  const children = ref<DefineComponent<typeof UCheckbox>[]>([]);

  const emitEvent = () => {
    let values: any[] = [];
    children.value?.map(val => {
      if(val.exposeProxy.modelValue) values.push(val.exposeProxy.name);
    });
    emit('change', values);
    ...
    setTimeout(() => {
      ...
      //   emitter.emit('on-form-change', values);
      // }
      dispatch('u-form-item', 'on-form-change', values);
    }, 60);
  };
