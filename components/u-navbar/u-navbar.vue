<template>
    ...
    >
      ...
      :style="{
        width: '100%',
        height: Number(navbarHeight) + state.statusBarHeight + 'px'
      }"
    ></view>
	</view>
</template>
...
<script setup lang="ts">
  ...
  const props = withDefaults(defineProps<UNavbarProps>(),{
    // 导航栏高度，单位px，非rpx
    height: '',
    // 返回箭头的颜色
  ...
  const { Z_INDEX_CONFIG, $parentUtil } = useComposable();

  const instance: ComponentInternalInstance | null = getCurrentInstance();

  const state = reactive<UNavbarState>({
  ...
  const navbarStyle = computed(() => {
    ...
    style.zIndex = Number(props.zIndex) ? Number(props.zIndex) : Z_INDEX_CONFIG.navbar;
    // 合并用户传递的背景色对象
  ...
  const navbarInnerStyle = computed(() => {
    ...
    // #ifdef MP
    let rightButtonWidth = systemInfo.windowWidth - (state.menuButtonInfo?.left || 0);
  ...
  const titleStyle = computed(() => {
    ...
    let rightButtonWidth = systemInfo.windowWidth - (state.menuButtonInfo?.left || 0);
    style.left = (systemInfo.windowWidth - uni.upx2px(Number(props.titleWidth))) / 2 + 'px';
    style.right = rightButtonWidth - (systemInfo.windowWidth - uni.upx2px(Number(props.titleWidth))) / 2 + rightButtonWidth +
      'px';
    // #endif
  ...
  const navbarHeight = computed(() => {
    ...
    // return menuButtonInfo.height + ((menuButtonInfo.top || 0) - this.statusBarHeight) * 2;//导航高度
  ...
  const goBack = () => {
    ...
    if (typeof props.customBack === 'function') {
      ...
      props.customBack.bind($parentUtil(instance?.proxy))();
    } else {
      uni.navigateBack({});
    }
  };
