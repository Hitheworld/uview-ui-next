...
<script setup lang="ts">
  ...
  import { useComposable } from '../../libs/hooks';
	 ...
	 */
  interface UNoNetworkState {
    isConnected: boolean; // 是否有网络连接
    networkType: string; // 网络类型
    isIOS: boolean;
  }
  interface UNoNetworkProps {
    // 页面文字提示
    tips?: string;
  ...
  const state = reactive<UNoNetworkState>({
  ...
  const uZIndex = computed(() => {
    return props.zIndex ? props.zIndex : Z_INDEX_CONFIG.noNetwork;
  });
  ...
  const gotoAndroidSetting = () => {
    const Intent: PlusAndroidClassObject = plus.android.importClass("android.content.Intent");
    const Settings = plus.android.importClass("android.provider.Settings");
    const mainActivity = plus.android.runtimeMainActivity();
    // @ts-ignore
  ...
  onMounted(() => {
    state.isIOS = (uni.getSystemInfoSync().platform === 'ios');
