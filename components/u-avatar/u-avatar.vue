  ...
  >
    ...
    >
      {{ uText }}
        ...
        class="u-avatar__sex"
        v-if="props.showSex"
        :class="['u-avatar__sex--' + props.sexIcon]"
        :style="[uSexStyle]"
    >
			<u-icon
          :name="props.sexIcon"
      ...
      />
        ...
        :style="[uLevelStyle]"
    >
			<u-icon :name="props.levelIcon" size="20" />
		</view>
	</view>
</template>
...
<script setup lang="ts">
  import { CSSProperties, computed, reactive, toRefs, watch } from "vue";
  ...
  const state = reactive<UAvatarState>({
    error: false,
    // 头像的地址，因为如果加载错误，需要赋值为默认图片，props值无法修改，所以需要一个中间值
  ...
  const wrapStyle = computed(() => {
    ...
    style.height = props.size == 'large' ? '120rpx' : props.size == 'default' ?
        '90rpx' : props.size == 'mini' ? '70rpx' : props.size + 'rpx';
