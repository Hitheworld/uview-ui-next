<template>
	<view
		...
		:class="{
			...
			'u-input--error': state.validateState
		}"
		:style="{
			padding: `0 ${props.border ? 20 : 0}rpx`,
	...
	>
      ...
      >
        ...
        />
			...
			<view
        class="u-input__right-icon__clear u-input__right-icon__item"
        v-if="props.passwordIcon && props.type == 'password'"
      >
				<u-icon
          size="32"
          :name="!state.showPassword ? 'eye' : 'eye-fill'"
          color="#c0c4cc" @click="state.showPassword = !state.showPassword"
        />
			</view>
			<view
...
<script setup lang="ts">
  ...
  interface UInputState {
    ...
    textareaHeight: number; // textarea的高度
    validateState: boolean; // 当前input的验证状态，用于错误时，边框是否改为红色
    focused: boolean; // 当前是否处于获得焦点的状态
    showPassword: boolean; // 是否预览密码
  ...
  const getStyle = computed(() => {
    ...
    style.minHeight = props.height ? props.height + 'rpx' : props.type == 'textarea' ?
      state.textareaHeight + 'rpx' : state.inputHeight + 'rpx';
  ...
  const handleInput = (event: any) => {
    ...
    // 过一个生命周期再发送事件给u-form-item，否则context.emit('update:modelValue')更新了父组件的值，但是微信小程序上
    // 尚未更新到u-form-item，导致获取的值为空，从而校验混论
    // 这里不能延时时间太短，或者使用nextTick，否则在头条上，会造成混乱
    setTimeout(() => {
  ...
  watch(() => props.modelValue, (newValue, oldValue) => {
    ...
    if(newValue != oldValue && props.type == 'select') handleInput({
      detail: {
        value: newValue
      }
    });
