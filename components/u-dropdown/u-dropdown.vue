      ...
      >
              ...
              :style="{
                color: item.disabled ? '#c0c4cc' :
                  (index === state.current || state.highlightIndex == index) ? props.activeColor : props.inactiveColor,
          ...
          >
            {{ item.title }}
              ...
              :class="{
                'u-dropdown__menu__item__arrow--rotate': index === state.current
              }"
          >
						<u-icon
                ...
                :color="index === state.current || state.highlightIndex == index ? props.activeColor : '#c0c4cc'"
            ></u-icon>
					</view>
				</view>
			</view>
        ...
        :style="[state.contentStyle, {
          transition: `opacity ${props.duration / 1000}s linear`,
...
<script setup lang="ts">
  ...
  import { useComposable } from '../../libs/hooks';
	 ...
	 */
  export interface UDropdownState {
    showDropdown: boolean; // 是否打开下来菜单,
    menuList: any[]; // 显示的菜单
    active: boolean; // 下拉菜单的状态
  ...
  const emit = defineEmits<{
    (e: 'open', current: number | string): void;
    (e: 'close', current: number | string): void;
  }>();

  const { addUnit, getRect, sys } = useComposable();

  // 引用所有子组件(u-dropdown-item)的instance，不能在data中声明变量，否则在微信小程序会造成循环引用而报错
  let children = [];

  const state = reactive<UDropdownState>({
  ...
  // 打开下拉菜单
  const open = (index: number | string) => {
    // 重置高亮索引，否则会造成多个菜单同时高亮
    // state.highlightIndex = 9999;
    // 展开时，设置下拉内容的样式
    state.contentStyle = {
      zIndex: 11,
    }
  ...
  const menuClick = (index: number) => {
    ...
    if (index === state.current && props.closeOnClickSelf) {
      ...
      setTimeout(() => {
        children[index].proxy.active = false;
      }, props.duration);
      return;
    }
    open(index);
  ...
  // 外部手动设置某个菜单高亮
  const highlight = (index = undefined) => {
  ...
  const getContentHeight = () => {
    ...
    // sys()为uView封装的获取设备信息的方法
    let windowHeight = sys().windowHeight;
    getRect('.u-dropdown__menu').then(res => {
