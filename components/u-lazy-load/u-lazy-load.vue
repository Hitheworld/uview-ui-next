<template>
    ...
    :style="{
      ...
      transition: `opacity ${state.time / 1000}s ease-in-out`
    }"
	  :class="'u-lazy-item-' + state.elIndex"
  >
		<view
      :class="'u-lazy-item-' + state.elIndex"
    >
			<image
        :style="{
          borderRadius: props.borderRadius + 'rpx', height: imgHeight
        }"
        v-if="!state.isError"
        class="u-lazy-item"
        :src="state.isShow ? props.image : props.loadingImg"
        :mode="props.imgMode"
        @load="imgLoaded"
        @error="loadError"
        @tap="clickImg"
...
<script setup lang="ts">
  ...
  import { useComposable } from '../../libs/hooks';
	 ...
	 */
  interface ULazyLoadState {
    isShow: boolean;
    opacity: number;
    time: number | string;
    loadStatus: 'lazyed' | 'loaded' | ''; // 默认是懒加载中的状态
    isError: boolean; // 图片加载失败
    elIndex: string;
  }
  interface ULazyLoadProps {
    index?: number | string;
    // 要显示的图片
    image: string;
    // 图片裁剪模式
    ...
    effect?: 'linear' | 'ease' | 'ease-in' | 'ease-out' | 'ease-in-out'| 'cubic-bezier(n,n,n,n)' | string;
    // 是否使用过渡效果
    isEffect?: boolean;
    // 圆角值
  ...
  const props = withDefaults(defineProps<ULazyLoadProps>(),{
    // index: null,
    // 要显示的图片
    image: '',
    // 图片裁剪模式
  ...
  const emit = defineEmits<{
    ...
    (e: 'load', index: string | number): void;
    (e: 'error', index: string | number): void;
  }>();

  const { guid, addUnit } = useComposable();
  ...
  const contentObserver = ref<IntersectionObserver | null>(null);

  const state = reactive<ULazyLoadState>({
    isShow: false,
    opacity: 1,
    time: props.duration,
  ...
  const disconnectObserver = (observerName: 'contentObserver') => {
    const observer = [observerName].value;
    observer && observer.disconnect();
  };

  watch(() => state.isShow, (newValue, oldValue) => {
    // 如果是不开启过渡效果，直接返回
  ...
  onMounted(() => {
    ...
    setTimeout(() => {
      ...
      const _contentObserver = uni.createIntersectionObserver(instance?.proxy);
      // 要理解这里怎么计算的，请看这个：
      ...
      contentObserver.value = _contentObserver;
    }, 30);
  });
